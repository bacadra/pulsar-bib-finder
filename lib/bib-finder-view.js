  ...
  constructor () {
    ...
    this.selectListView = new SelectListView({
      ...
      elementForItem: (item) => {
        ...
        matches = query.length>0 ? zadeh.match(item.key, query) : []
        ...
        matches = query.length>0 ? zadeh.match(item.description, query) : []
        this.highlightMatchesInElement(item.description, matches, secBlock)
      ...
      filter: (items, query) => {
        ...
        for (let item of items) {
          item.score  = zadeh.score(item.key, query)
          item.score += zadeh.score(item.description, query)
    ...
    for (fPath of paths) {
      try {
        ...
        for(let entry of entries) {
          ...
          this.items.push({key: entry.key, description: description})
