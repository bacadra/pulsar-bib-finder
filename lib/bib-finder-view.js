...
export default class BibFinderView {
  ...
  async show (id) {
    ...
    if (id===1 || id===undefined){
      path = atom.config.get('bib-finder.bibPaths.path1')
      if (path) {paths.push(path)}
    }
    if (id===2 || id===undefined){
      path = atom.config.get('bib-finder.bibPaths.path2')
      if (path) {paths.push(path)}
    }
    if (id===3 || id===undefined){
      path = atom.config.get('bib-finder.bibPaths.path3')
      if (path) {paths.push(path)}
    }
    if (id===4 || id===undefined){
      path = atom.config.get('bib-finder.bibPaths.path4')
      if (path) {paths.push(path)}
    }
    if (id===5 || id===undefined){
      path = atom.config.get('bib-finder.bibPaths.path5')
      if (path) {paths.push(path)}
    }
    ...
    for (path of paths) {
      if (!fs.existsSync(path)) {
        atom.notifications.addError(`The bib file ${path} does not exists`)
        continue
      }
      for(let entry of bibParser.getBibAsObject(path)) {
        content.push({
          name       : entry.key,
