...
export default class BibFinderView {
  ...
  constructor () {
    this.selectListView = new SelectListView({
      ...
      infoMessage: 'Press `Enter`, `Alt-Enter` or `Ctrl-Enter` to insert entry',
      ...
      didConfirmSelection: (item) => {
        ...
        if (this.insMode===1) {
          ...
          for (cursor of cursors) {
            cursor.moveLeft(item.name.length+3)
          }
        }
      },
  ...
  async show (id) {
    ...
    if (id===undefined && (gloPaths=atom.config.get('bib-finder.bibPaths.array'))) {
      paths.push(...gloPaths)
    }


    if (id===undefined && atom.config.get('bib-finder.bibLocal')) {
      cwdZero = cwd()
    ...
    for (pathFile of paths) {
      try {
        ...
        text = fs.readFileSync(pathFile, 'utf-8');
