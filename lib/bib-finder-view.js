...
export default class BacadraBibView {
  ...
  async show () {
    ...
    path1 = atom.config.get('bib-finder.bibPath1')
    if (path1!='') {paths.push(path1)}

    path2 = atom.config.get('bib-finder.bibPath2')
    if (path2!='') {paths.push(path2)}

    path3 = atom.config.get('bib-finder.bibPath3')
    if (path3!='') {paths.push(path3)}

    path4 = atom.config.get('bib-finder.bibPath4')
    if (path4!='') {paths.push(path4)}

    path5 = atom.config.get('bib-finder.bibPath5')
    ...
    for (path of paths) {
      ...
      for (let i = 0; i < bibArr.length; i++) {
        ...
        content.push({
          name:row.key,
          displayName:row.key + ' | '+ row.entryType + ' | ' + Object.values(row.data).join(' | ')
        })
      }
    }
