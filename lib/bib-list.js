...
export default class BibList {
  ...
  constructor (S) {
    ...
    this.slist.element.classList.add('bib-finder')
    ...
    this.disposables.add(
      ...
      atom.config.observe('bib-finder.showSource', (value) => {
        this.showSource = value ; this.slist.update()
      }),
  ...
  update(id) {
    if (!this.items || this.reloadAlways || this.id!=id) {
      this.slist.update({ items:[], loadingMessage:this.getLoadingMessage(), infoMessage:null })
      this.cache(id).then(() => {
        this.slist.update({ items:this.items, loadingMessage:null, infoMessage:this.getInfoMessage() })
      })
    }
  }
  ...
  cache(id) {
    return new Promise((resolve, reject) => {
      ...
      this.objectArrayFilterer.setCandidates(this.items, 'text',
        { usePathScoring:false, useExtensionBonus:false })
      resolve()
    })
  }
  ...
  didConfirmSelection(item, mode) {
    if (!item) { item = this.slist.getSelectedItem() }
  ...
  filter(items) {
    ...
    return this.objectArrayFilterer.filter(this.query, { maxResults: 1000 })
  }
}
