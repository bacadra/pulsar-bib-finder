  ...
  constructor (S) {
    ...
    this.slv = new SelectListView({
      ...
      elementForItem: (item, {_, index, visible}) => {
        ...
        let matches = query.length>0 ? zadeh.match(Diacritics.clean(item.text), query) : []
        ...
        if (this.showSource) {
          ...
          pathBlock.classList.add('secondary-line', 'icon')
          if (this.S.addIconToElement) {
            this.S.addIconToElement(pathBlock, item.fPath)
          } else {
            priBlock.classList.add('icon-file-text')
          }
          pathBlock.innerHTML = item.fPath
      ...
      filter: (items, query) => {
        ...
        for (let item of items) {
          ...
          item.score += zadeh.score(item.key, query)*100
